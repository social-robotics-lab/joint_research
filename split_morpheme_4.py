...
ただし、名詞、動詞、形容詞のみを出力するようにフィルタリングしています。
Janomeのインストール方法: pip install janome
入力ファイルの形式は以下のように、タブ区切りで複数列があるとします。
列1    列2    列3...    分かち書きするテキスト
出力ファイルの形式は以下のようになります。
列1    列2    列3...    分かち書きしたトークンの1つめ    品詞
列1    列2    列3...    分かち書きしたトークンの2つめ    品詞
列1    列2    列3...    分かち書きしたトークンの3つめ    品詞
...
"""
...
with open(input_file, "r", encoding="utf-8") as f_in, \
    ...
    for line in f_in:
        ...
        if cols[-3]:
            ...
            for token in tokens:
                pos = token.part_of_speech.split(",")[0]  # 品詞の最初の要素を取得
                if pos in ["名詞", "動詞", "形容詞"]:  # 名詞、動詞、形容詞のみ出力
